...
const AddressUpdatePage = ({ match }) => {
    ...
    const addressSubmitHandler = (e) => {
        ...
        const finalPhone = phoneNumber || (address ? address.phone_number : "");
        
        let formattedPhone = finalPhone;
        // If it's a 9 digit number without +998, prepend it
        if (/^[0-9]{9}$/.test(finalPhone)) {
            formattedPhone = `+998${finalPhone}`;
        }
        
        if (!/^\+998[0-9]{9}$/.test(formattedPhone)) {
            toast.error("Iltimos, telefon raqamini to'g'ri kiriting (Masalan: +998901234567)");
            return;
        }
        ...
        const updatedAddress = {
            ...
            "pin_code": pinCode || (address ? address.pin_code : ""),
            "house_no": houseNumber || (address ? address.house_no : ""),
            "landmark": landmark || (address ? address.landmark : ""),
