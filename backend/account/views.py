...
class AdminDashboardView(APIView):
    ...
    def get(self, request):
        ...
        total_revenue = OrderModel.objects.aggregate(Sum('total_price'))['total_price__sum'] or 0
        
        # Product modulidan chaqirish
        from product.models import Product
        total_products = Product.objects.count()
        
        recent_orders = OrderModel.objects.order_by('-id')[:5]
        serializer = AllOrdersListSerializer(recent_orders, many=True)
...
class AdminUserUpdateView(APIView):
    ...
    def put(self, request, pk):
        try:
            ...
            user.is_superuser = data.get("is_admin", user.is_superuser)
            user.save()
            serializer = UserSerializer(user, many=False)
            return Response(serializer.data, status=status.HTTP_200_OK)
        except Exception:
    ...
    def delete(self, request, pk):
        try:
            ...
            if user.id == request.user.id:
                return Response({"details": "You cannot delete yourself."}, status=status.HTTP_400_BAD_REQUEST)
            user.delete()
            return Response({"details": "User successfully deleted."}, status=status.HTTP_204_NO_CONTENT)
        except Exception:
            return Response({"details": "User not found."}, status=status.HTTP_404_NOT_FOUND)
